...
DROP POLICY IF EXISTS "Authenticated users can manage taxonomy orders" ON public.taxonomy_orders;

CREATE POLICY "Admins can manage photos" ON public.photos FOR ALL TO authenticated USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Admins can manage taxonomy orders" ON public.taxonomy_orders FOR ALL TO authenticated USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
